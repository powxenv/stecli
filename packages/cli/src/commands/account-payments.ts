...
import { getPayments } from "#/services/horizon.js";
...
export const accountPayments = defineCommand({
  ...
  async run({ args }) {
    ...
    await runCommand(async () => {
      const paymentsResult = await getPayments(address, network, {
        limit: Number(args.limit) || 10,
        cursor: args.cursor != null ? String(args.cursor) : undefined,
        order: args.order === "asc" ? "asc" : "desc",
      });
      if (Result.isError(paymentsResult))
        return Result.err(formatHorizonError(paymentsResult.error));

      return Result.ok({
        address,
        count: paymentsResult.value.length,
